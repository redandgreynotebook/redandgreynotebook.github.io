---
title: ok now do it in ocaml
tags: puzzle
comments: true
---

honestly, with the amount of recursive backtracking this thing has, it'd probably perform better in ocaml. 
...
let solve_column config column = you get the point
```
doing this leaves us with only 1005 valid configurations, out of around 330 million total. that's $3 \cdot 10^{-4}$ percent of total configurations that are valid, a huge cutdown of search space. 
...
for completeness, here's our full implementation in c++. it takes around a minute to solve in [replit](https://replit.com), taking up 100mb of ram. to see just how optimized `forced_grid` makes it, a `solve_config` without it takes around 500-1500 seconds, while a `solve_config` forcing cells first takes at most 1.5 seconds. 
