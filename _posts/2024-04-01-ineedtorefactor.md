...
well, didn't we [do this before](/posts/js0623)? almost -- the constraints were different, but our general procedure for partitioning the grid, creating configurations, and high-level backtracking should be fine. it remains to modify the small-scale solving functions, namely `forced_board`. instead of worrying about each row and column, we instead try and solve each constraint separately, then taking the intersection of these solutions to see which cells are forced (hint: it's a lot of them). we'll be using terms brought over from the previous puzzle, so take a look at that first for a refresher.
